    ...
    st.markdown("""
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.5rem;">
            <div style="display: flex; align-items: center; gap: 0.75rem;">
                <span style="font-size: 20px; cursor: pointer; color: var(--text-secondary);">[Menu]</span>
...
def render_ledger_panel(ledger: Ledger):
    ...
    for rule in ledger.rules:
        ...
        with col2:
            if st.button("[Edit]", key=f"edit_btn_{rule.id}"):
                st.session_state[edit_key] = True
                st.rerun()

        with col3:
            if st.button("[x]", key=f"del_btn_{rule.id}"):
    ...
    for i, criterion in enumerate(ledger.rubric.criteria):
        ...
        with col2:
            if st.button("[Edit]", key=f"edit_btn_crit_{i}"):
                st.session_state[edit_key] = True
                st.rerun()

        with col3:
            if st.button("[x]", key=f"del_btn_crit_{i}"):
